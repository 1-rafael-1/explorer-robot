...
//!
//! This module provides a dedicated task for reading encoder pulse counts from both motors.
...
//! let pulses = measurement.left;  // This is the delta!
//! ```
//!
//! # Encoder Hardware
//!
//! - **Type**: Single-channel pulse counting (not quadrature)
...
//! - **Pins**: Left encoder GPIO 7, Right encoder GPIO 8
//!
...
///
/// Contains pulse counts from both motors and a timestamp.
...
/// Encoder channels for both motors
struct EncoderChannels {
    /// Encoder input for left motor
    left: Pwm<'static>,
    /// Encoder input for right motor
    right: Pwm<'static>,
}
...
impl EncoderChannels {
    ...
    fn read_all(&self) -> EncoderMeasurement {
        ...
        EncoderMeasurement {
            left: self.left.counter(),
            right: self.right.counter(),
            timestamp_ms,
        }
    }
}
...
/// Supports start/stop and manual reset.
///
/// # Parameters
/// - `encoder_left`: PWM channel configured as encoder input for left motor (GPIO 7)
/// - `encoder_right`: PWM channel configured as encoder input for right motor (GPIO 8)
#[embassy_executor::task]
pub async fn encoder_read(encoder_left: Pwm<'static>, encoder_right: Pwm<'static>) {
