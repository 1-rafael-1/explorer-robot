...
/// Map a test menu index to its selection (v3: 6 tests).
/// Returns `None` when the selection is the Back entry.
pub const fn test_selection_from_index(index: usize) -> Option<TestSelection> {
    match index {
        0 => Some(TestSelection::BasicMotor),
        1 => Some(TestSelection::Turns),
        2 => Some(TestSelection::StraightDrive),
        3 => Some(TestSelection::ArcDrive),
        4 => Some(TestSelection::Imu6Axis),
        5 => Some(TestSelection::Imu9Axis),
        _ => None,
    }
}
